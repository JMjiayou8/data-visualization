...
var getWholeMonth = function (start) {
  ...
  if (jump >= 12) {
    for (var i = 0; i < jump + 1; i++) {
      t = i + month - 1;
      re.push((year + parseInt(t / 12)) + '-' + addZero(t % 12 + 1))
    }
  } else {
    for (var i = jump - 12 + 1; i < jump + 1; i++) {
      t = i + month - 1;
      re.push((year + parseInt(t / 12)) + '-' + addZero(t % 12 + 1))
    }
  }
...
function rendList (month, offset) {
  ...
  for (var i = 0; i < wholeMonthList.length; i++) {
    if (new Date(wholeMonthList[i]) >= new Date(startMonth)) {
      validArr.push(wholeMonthList[i])
    }
  }
  for (var i = 0; i < showMonthList.length; i++) {
    var item = showMonthList[i];
    listArr.push('<li class="timeLine-item ' + (item == activeMonth ? 'active' : '') + ' ' + ((new Date(item) < new Date(startMonth)) ? 'disabled' : '') + ' " onclick="clickMonth(\'' + item + '\');"><p>' + item + '</p><i></i></li>')
  }
